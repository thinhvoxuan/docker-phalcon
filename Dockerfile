...
RUN /usr/bin/apt-get update && apt-get -y install git build-essential curl php5-xdebug php5-dev php5-curl php5-mysqlnd php5-cli php5-gd imagemagick php5-imagick php5-mcrypt php5-intl libpcre3-dev gcc make vim && \
...
RUN /bin/echo "xdebug.remote_enable=1" >> /etc/php5/mods-available/xdebug.ini
RUN /bin/echo "xdebug.remote_host=10.254.254.254" >> /etc/php5/mods-available/xdebug.ini
RUN /bin/echo "xdebug.remote_port=9001" >> /etc/php5/mods-available/xdebug.ini
RUN /bin/echo "xdebug.idekey=\"PHPSTORM\"" >> /etc/php5/mods-available/xdebug.ini
RUN /bin/echo "xdebug.remote_autostart=1" >> /etc/php5/mods-available/xdebug.ini
